...
if (query && query.refresh && query.refresh === 'true') { isPostAuthRedirect.value = true }

const redirectTo = ref(query.redirectTo)
if (!redirectTo.value || redirectTo.value === '') { redirectTo.value = '/tickets' }
...
async function signInWithEmail (email: string) {
  ...
  if (email) {
    ...
    if (error) { emailOptInError.value = error } else { emailOptInError.value = '' }
  }
}
        ...
        <div v-else class="ml-0 sm:ml-10 w-full sm:w-[275px]">
          <form method="POST" @submit.prevent="submitForm">
            <input
              ...
              name="firstname"
              type="firstname"
              class="w-full border rounded-md py-1 px-2 mr-2 mt-1"
