...
const { path } = useRoute()
const { data } = await useAsyncData(`content-${path}`, () => queryContent(localePath('/blog')).sort({ date: -1 }).limit(props.nrOfPosts).find())
...
<template>
  <div v-for="post, index in data" :key="index" class="w-full md:w-2/3 lg:w-1/2 xl:w-1/3 px-4 animated fadeIn" :class="animateDurationClass(index)">
